...
const Order = () => {
  const order = useLoaderData(); // Everyone can search for all orders, so for privacy reasons we're gonna gonna exclude names or address, these are only for the restaurant staff
  const {
    id,
    status,
    priority,
  ...
  return (
    <div className="space-y-8 px-4 py-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 bg-stone-200 px-6 py-5">
        ...
        <p className='text-xm text-stone-500'>(Estimated delivery: {formatDate(estimatedDelivery)})</p>
      </div>
